...
DJVuMetadata::DJVuMetadata(const fs::path &path, const bool read_only)
  ...
  djvu_doc = DjVuDocEditor::create_wait(GURL::Filename::UTF8(path.string().c_str()));
...
void DJVuMetadata::write_to_disk() {
  ...
  auto bzz = BSByteStream::create(ann_bs, ser_annotations.length());
